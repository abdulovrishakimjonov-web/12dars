          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-10 place-items-center'>
            {
              data.map((item) => (
                <div
                  ...
                  className='w-full max-w-[350px] rounded-lg border-2 border-gray-300 flex flex-col p-5 sm:p-7 relative cursor-pointer'
                >
                  <button
                  ...
                  <img className='mb-2' src={item.avatar || qopcha} alt={item.malumot || "product"} />

                  <div className='flex items-center gap-2 text-gray-400 underline mb-2 text-sm'>
                  ...
                  <div className='flex items-center justify-between'>
                    <p className='text-xl sm:text-2xl font-bold text-[#1F2533]'>{item.price ? `$${item.price}` : '$12.56'}</p>
                    <button onClick={(e) => e.stopPropagation()} className='border-2 border-gray-300 text-[#359740] cursor-pointer w-[39px] flex justify-center p-2 rounded-md hover:bg-emerald-100 transition-all'>
